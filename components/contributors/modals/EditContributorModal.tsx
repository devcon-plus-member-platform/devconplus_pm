...
export default function EditContributorModal({ contributor, roles, onSave, onClose }: Props) {
  const [fullName, setFullName] = useState(contributor.full_name ?? "");
  const [email, setEmail] = useState(contributor.email);
  const [roleId, setRoleId] = useState<string>(contributor.role_id ?? "");
  const [telegram, setTelegram] = useState(contributor.telegram_username ?? "");
  ...
  return (
    <Modal open onClose={onClose} title="Edit Contributor">
